...
class IAMasterService:
    ...
    async def create_ia_entry(
        ...
        # 1. Check for existing record (Upsert logic)
        ...
        # 2. Save IA Files (Only if provided)
        ...
        # 3. Create or Update IA Master Record
        if existing_ia:
            # Update existing
            for key, value in ia_data.items():
                if value is not None: # Avoid resetting if form somehow sends nulls for existing values
            ...
            # Clear old employees to re-sync (Simplest way for Master data entry)
            db.query(EmployeeDetails).filter(EmployeeDetails.ia_master_id == db_ia.id).delete()
        else:
            # Create new
        ...
        # We re-add from the employees_json which contains existing paths too
        for i, emp_data in enumerate(employees_data):
            emp_data['ia_master_id'] = db_ia.id
            
            # Check for new certificate upload
        ...
        self.audit_repo.log_event(
            db, action, "ia_master", str(db_ia.id), 
    ...
    async def sign_file_urls(
        ...
        """
        Converts internal paths (local or cloud) into accessible URLs.
        Uses the provided DB session (Client DB) to look up storage keys.
        """
        if not ia_record:
            return ia_record
    ...
    async def get_latest_ia(
        ...
        if db_ia:
            ...
            # Sign URLs using the client DB session directly
