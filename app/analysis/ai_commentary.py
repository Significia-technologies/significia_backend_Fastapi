...
class SystemCommentaryGenerator:
    ...
    def generate_executive_brief(client_name, hlv_income, hlv_expense, retirement_corpus,
                                 ...
                                 monthly_investments, net_worth, savings_rate, medical_inflation) -> str:
        ...
        <div class="analysis-card" style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border-left: 4px solid #0ea5e9;">
            ...
            <h5><strong>Methodology & Approach</strong></h5>
            <p> This analysis follows a conservative financial analysis approach. All goal projections are based on inflation-adjusted costs. Monthly contribution requirements are computed without dependence on inventment return assumptions for funding. Any investment returns, if realized, may support or accelerate goal achievement but are not considered necessary for meeting projected requirements</p>
            ...
            <p>The analysis presents two HLV calculation methodologies: income replacement covering {years_to_retirement} years until retirement, and expense replacement based on spouse life expectancy considerations. Medical inflation at {medical_inflation}% annually is factored into healthcare cost projections. Retirement planning assumes {savings_rate}% current savings rate with pre-retirement return assumptions. Child education and marriage goals are evaluated with current cost estimates and time horizons.</p>

            <h5><strong>KEY INFORMATION HIGHLIGHTS</strong></h5>
            <p>Insurance analysis reveals current coverage levels relative to calculated HLV values. Medical coverage assessment includes current cover amount and bonus accumulation features. Retirement readiness is measured against target corpus requirements. Child education and marriage goals are quantified with inflation-adjusted future values. Existing asset allocation includes designated percentages for specific child goals.</p>
        </div>
        """

    @staticmethod
    def generate_hlv_comments(hlv_income, net_hlv, existing_assets,
                              current_liabilities, current_cover, 
                              years_to_retirement, sol_hlv) -> list:
        """Generate 4 comments for HLV analysis."""
        asset_buffer = int((existing_assets / hlv_income * 100)) if hlv_income > 0 else 0
        return [
            f"<strong>Insight 1:</strong> HLV requirements assume lifestyle maintenance at {sol_hlv}% of current expense.",
            f"<strong>Insight 2:</strong> The gross HLV of Rs {hlv_income:,} represents {years_to_retirement} years of income replacement.",
            f"<strong>Insight 3:</strong> Existing financial assets of Rs {existing_assets:,} provide a {asset_buffer}% buffer against total protection needs, resulting in net insurance requirement of Rs {net_hlv:,}.",
            f"<strong>Insight 4:</strong> Current liabilities of Rs {current_liabilities:,} are factored into the net HLV calculation.",
            f"""<strong>Insight 5: Please Note the Following Assumptions in Expense Method HLV Calculation made:</strong>
            <ul>
                <li>Safety Margin: It ensures that even if the family keeps the money in a basic savings account or if the markets perform poorly, their expenses are covered.</li>
                <li>Worst-Case Scenario: Life insurance is a safety net. By not discounting the expenses back using an 8% return, the calculator generates a higher, safer requirement.</li>
                <li>Existing financial asset has been calculated  excluding Land/ Building and Education/Marriage fund as building is blocked non financial asset and education/marriage fund is already earmarked for specific goals.</li>
            </ul>""",
        ]
    ...
    def generate_medical_comments(medical_retirement, coverage_retirement,
                                  ...
                                  balance_needed_life_expectancy, medical_inflation) -> list:
    ...
    def generate_retirement_comments(corpus_retirement, monthly_investment,
                                     years_remaining, readiness_score, sol_ret) -> list:
    ...
    def generate_child_goal_comments(education_future, marriage_future,
                                     ...
                                     monthly_investment_marriage, inflation) -> list:
        """Generate 3 comments for child goal analysis."""
        return [
            f"<strong>Insight 1:</strong> Future education corpus requirement of Rs {education_future:,} reflects projected education inflation of {inflation}% annually.",
    ...
    @staticmethod
    def generate_cash_flow_analysis(post_ret_rate) -> list:
        ...
        return [
            ...
            f"<strong>Insight 3:</strong> Investment growth during retirement years utilizes a conservative {post_ret_rate}% return assumption.",
            "<strong>Insight 4:</strong> Portfolio depletion is mathematically engineered to reach zero balance at life expectancy.",
            "<strong>Insight 5:</strong> The analysis incorporates longevity risk considerations.",
            "<strong>Insight 6:</strong> Cash flow sequencing addresses sequence-of-returns considerations.",
            "<strong>Insight 7:</strong> Terminal year adjustments ensure complete fund utilization.",
    ...
    def generate_overall_conclusion(client_name, financial_score,
                                    ...
                                    hlv_gap_income, hlv_gap_expense, medical_gap_retirement, years_remaining,
                                    ...
                                    years_to_retirement, inflation, medical_inflation) -> str:
        ...
        <div class="analysis-card" style="background: linear-gradient(135deg, #fefce8 0%, #fef9c3 100%); border-left: 4px solid #ca8a04;">
            ...
            <p>This analysis presents a comprehensive evaluation of financial objectives. The analysis is based on a conservative, long-return-dependent methodology. Insurance protection evaluation indicates gaps of Rs {min(hlv_gap_income, hlv_gap_expense):,} based on selected methodology. Medical inflation at {medical_inflation}% annually versus general inflation at {inflation}% creates differential in healthcare purchasing power projections. Retirement corpus assessment shows gap of Rs {retirement_gap:,} requiring systematic monthly investments over {years_remaining:,} years. Child education (Rs {child_education_gap:,}) and marriage (Rs {child_marriage_gap:,}) gaps are presented with corresponding time horizons. Current Rs {net_worth:,} net worth and {savings_rate}% savings rate form the foundation for financial planning.</p>
    ...
    def generate_all_commentary(calculations: dict, hlv_data: dict,
        ...
        result['executive_brief'] = SystemCommentaryGenerator.generate_executive_brief(
            ...
            medical_inflation=calculations.get('medical_inflation', 10),
        )
        ...
        result['hlv_comments'] = SystemCommentaryGenerator.generate_hlv_comments(
            ...
            years_to_retirement=calculations.get('years_to_retirement', 0),
            sol_hlv=calculations.get('sol_hlv', 80)
        )
        ...
        result['retirement_comments'] = SystemCommentaryGenerator.generate_retirement_comments(
            ...
            sol_ret=calculations.get('sol_ret', 80)
        )
        ...
        result['child_goal_comments'] = SystemCommentaryGenerator.generate_child_goal_comments(
            ...
            inflation=calculations.get('inflation', 6),
        )
        ...
        result['cash_flow_analysis'] = SystemCommentaryGenerator.generate_cash_flow_analysis(
            post_ret_rate=calculations.get('post_ret_rate', 8)
        )
